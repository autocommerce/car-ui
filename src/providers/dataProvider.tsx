...
const dataProvider: DataProvider = {
  ...
  getOne: async (resource, params) => {
    if (resource === "users") {
      const { data } = await accountsProvider.getOne(params.id.toString());
  ...
  update: async (resource, params) => {
    if (resource === "users") {
      const { data } = await accountsProvider.update(params.id, params.data as User);
      return { data: { ...data, id: data.id } };
    }
    return Promise.reject("Unknown resource");
  },
  delete: async (resource, params) => {
    if (resource === "users") {
      const { data } = await accountsProvider.delete(params.id.toString());
      return { data: { ...data, id: data.id } };
    }
    return Promise.reject("Unknown resource");
  },
